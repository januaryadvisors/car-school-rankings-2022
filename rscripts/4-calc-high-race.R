#Calculate HIGH school rankings with RACE EQUITY
library(tidyverse)
library(janitor)
library(here)
library(broom)
...
high <- campus_master %>%
  ...
  mutate(
    ...
    eng2.meets.cnt.2_soc = ifelse(is.na(eng2.meets.cnt.2_soc),eng1.meets.cnt.2_soc, eng2.meets.cnt.2_soc), #if no eng2, replace with eng1; makes sure w.avg isn't NA
    eng2.meets.cnt.2_ecod = ifelse(is.na(eng2.meets.cnt.2_ecod),eng1.meets.cnt.2_ecod, eng2.meets.cnt.2_ecod), #if no eng2, replace with eng1; makes sure w.avg isn't NA
    
    e.avg = ((eng1.meets.cnt.2 + eng2.meets.cnt.2)/(eng1.all.cnt.2 + eng2.all.cnt.2))*100, #generate weighted meets percentage
    e.avg_soc = ((eng1.meets.cnt.2_soc + eng2.meets.cnt.2_soc)/(eng1.all.cnt.2_soc + eng2.all.cnt.2_soc))*100, #generate weighted meets percentage
    e.avg_ecod = ((eng1.meets.cnt.2_ecod + eng2.meets.cnt.2_ecod)/(eng1.all.cnt.2_ecod + eng2.all.cnt.2_ecod))*100, #generate weighted meets percentage
    
    m.avg = ((alg1.meets.cnt.2)/(alg1.all.cnt.2))*100,
    m.avg_soc = ((alg1.meets.cnt.2_soc)/(alg1.all.cnt.2_soc))*100,
    m.avg_ecod = ((alg1.meets.cnt.2_ecod)/(alg1.all.cnt.2_ecod))*100,
    
    w.avg = mean(c(e.avg, m.avg), na.rm = T),
    w.avg_soc = mean(c(m.avg_soc, e.avg_soc), na.rm = T),
    w.avg_ecod = mean(c(m.avg_ecod, e.avg_ecod), na.rm = T),
...
## RACE EQUITY INDEX -----------------------
soc.relative.performance.elem <- loess(w.avg ~ soc.pct, high)
...
tabyl(high, race.equity.index.grade)
...
high <- high %>%
  mutate(
    overall.score = (w.avg + camp.perf.score + growth.avg + coll.ready.score + race.equity.index)/5,
...
high_original = read_csv(here::here("output/high_2022_all_state.csv")) %>% 
...
high_final <- high %>%
  ...
  dplyr::select(
    ...
    race.equity.index.grade,
    charter, county, low.grade, high.grade, region,
    growth.avg, w.avg, camp.perf.score, coll.ready.score, race.equity.index,
  ...
  left_join(., high_original) %>% 
  dplyr::select("Rank", "Rank w/o Race Equity", everything())


write_csv(high_final, file=here::here("output", "high_2022_all_state_race.csv"))
...
lapply(1:length(regions), function(x) {
  ...
  write_csv(df, file=here::here("output", paste0("high_2022_all_", region_export_name, '_race.csv')))
})

# Gold Ribbon tables ----------------------
gr_all = filter(high_final, `Gold Ribbon School?`=="Yes")
write_csv(gr_all, file=here::here("output", "high_2022_goldribbon_state_race.csv"))
...
lapply(1:length(regions), function(x) {
  ...
  write_csv(df, file=here::here("output", paste0("high_2022_goldribbon_", region_export_name, '_race.csv')))
})
